...
class CleanTestCoursesCommand extends Command
{
    protected static $defaultName = 'app:clean-test-courses';
    ...
    {
        $this->setDescription('Delete test courses and update remaining with real photos');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        // Delete test courses (handle foreign key constraints)
        $testCourseIds = [10, 11, 14, 16, 18, 19];
        $output->writeln('Deleting test courses...');
        
        // First delete related records (in correct order for foreign keys)
        foreach ($testCourseIds as $id) {
            // Delete quiz results first (references quiz)
            $conn->executeStatement('DELETE FROM quiz_result WHERE quiz_id IN (SELECT id FROM quiz WHERE course_id = ?)', [$id]);
            $conn->executeStatement('DELETE FROM quiz_attempt WHERE quiz_id IN (SELECT id FROM quiz WHERE course_id = ?)', [$id]);
            $conn->executeStatement('DELETE FROM course_test_result WHERE course_test_id IN (SELECT id FROM course_test WHERE course_id = ?)', [$id]);
            $conn->executeStatement('DELETE FROM certificate WHERE course_id = ?', [$id]);
            $conn->executeStatement('DELETE FROM enrollment WHERE course_id = ?', [$id]);
            $conn->executeStatement('DELETE FROM quiz WHERE course_id = ?', [$id]);
            $conn->executeStatement('DELETE FROM course_test WHERE course_id = ?', [$id]);
            $conn->executeStatement('DELETE FROM lesson_completion WHERE lesson_id IN (SELECT l.id FROM lesson l JOIN chapter ch ON l.chapter_id = ch.id WHERE ch.course_id = ?)', [$id]);
            $conn->executeStatement('DELETE FROM lesson WHERE chapter_id IN (SELECT id FROM chapter WHERE course_id = ?)', [$id]);
            $conn->executeStatement('DELETE FROM chapter WHERE course_id = ?', [$id]);
        }
        
        // Now delete the courses
        foreach ($testCourseIds as $id) {
            $conn->executeStatement('DELETE FROM course WHERE id = ?', [$id]);
        }
        $output->writeln('Deleted ' . count($testCourseIds) . ' test courses');
        ...
        // Update all courses with appropriate images based on category
        foreach ($categoryImages as $categoryName => $imageUrl) {
            ...
            $result = $conn->executeStatement($sql, ['url' => $imageUrl, 'categoryName' => $categoryName]);
            if ($result > 0) {
        ...
        $result = $conn->executeStatement($sql, ['url' => $defaultImage]);
        $updated += $result;
