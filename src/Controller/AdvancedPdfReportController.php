...
class AdvancedPdfReportController extends AbstractController
{
        ...
        return new Response(
            ...
            [
                ...
                'Content-Disposition' => sprintf('attachment; filename="rapport-quiz-%s-%s.pdf"', 
                    $quizResult->getUser()?->getFullName() ?? 'Unknown',
                    $quizResult->getQuiz()?->getTitle() ?? 'Quiz'
                )
            ]
        );
    }
    ...
    {
        $createdAt = $quizResult->getCreatedAt();
        $createdAtStr = $createdAt !== null ? $createdAt->format('Y-m-d H:i:s') : 'unknown';
        $verificationUrl = $this->generateUrl('quiz_verification', [
            'resultId' => $quizResult->getId(),
            'token' => md5($quizResult->getId() . $createdAtStr)
