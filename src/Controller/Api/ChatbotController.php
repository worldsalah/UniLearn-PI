...
class ChatbotController extends AbstractController
{
    ...
    private function getUserContext(): array
    {
        ...
        return [
            ...
            'is_freelancer' => in_array('ROLE_FREELANCER', $user->getRoles(), true),
            'is_client' => in_array('ROLE_CLIENT', $user->getRoles(), true)
        ];
    }
...
Now provide your expert recommendation:";
    ...
    private function analyzeUserNeeds(string $message): array
    {
        ...
        if (preg_match('/web|site|app|development|dev|programming|coding/i', $message) !== false) {
            $needs[] = 'web development';
        }
        if (preg_match('/design|ui|ux|graphic|logo|brand/i', $message) !== false) {
            $needs[] = 'design services';
        }
        if (preg_match('/writing|content|blog|article|copy/i', $message) !== false) {
            $needs[] = 'content creation';
        }
        if (preg_match('/marketing|seo|social|ads|promotion/i', $message) !== false) {
            $needs[] = 'digital marketing';
        }
        if (preg_match('/video|course|tutorial|education|learn/i', $message) !== false) {
            $needs[] = 'educational content';
        }
        if (preg_match('/cheap|budget|affordable|low cost|inexpensive/i', $message) !== false) {
            $needs[] = 'budget-friendly options';
        }
        if (preg_match('/urgent|quick|fast|asap|immediate/i', $message) !== false) {
            $needs[] = 'quick delivery';
        }
        if (preg_match('/professional|expert|premium|quality|best/i', $message) !== false) {
    ...
    private function detectPriceSensitivity(string $message): string
    {
        ...
        if (preg_match('/cheap|budget|affordable|low cost|inexpensive|discount|save/i', $message) !== false) {
            return 'HIGHLY PRICE SENSITIVE';
        } elseif (preg_match('/reasonable|moderate|mid-range/i', $message) !== false) {
            return 'PRICE CONSCIOUS';
        } elseif (preg_match('/premium|expensive|quality|best|professional/i', $message) !== false) {
    ...
    private function detectUrgency(string $message): string
    {
        ...
        if (preg_match('/urgent|asap|immediate|quick|fast|today|now|emergency/i', $message) !== false) {
            return 'EXTREME URGENCY';
        } elseif (preg_match('/soon|this week|next few days/i', $message) !== false) {
            return 'HIGH URGENCY';
        } elseif (preg_match('/considering|thinking|maybe|planning/i', $message) !== false) {
    ...
    private function detectQualityPreference(string $message): string
    {
        ...
        if (preg_match('/professional|expert|premium|quality|best|top-rated|experienced/i', $message) !== false) {
            return 'SEEKS PREMIUM QUALITY';
        } elseif (preg_match('/good|reliable|solid|decent/i', $message) !== false) {
            return 'WANTS GOOD QUALITY';
        } elseif (preg_match('/basic|simple|beginner|starter/i', $message) !== false) {
