...
class PaginationApiController extends AbstractController
{
    ...
    public function getPaginatedProducts(Request $request): JsonResponse
    {
        ...
        try {
            ...
            // Apply sorting
            $allowedSortFields = ['createdAt', 'title', 'price', 'id'];
        ...
        try {
            ...
            if ($startDate !== null && $startDate !== '') {
                $queryBuilder->andWhere('o.createdAt >= :startDate')
                    ->setParameter('startDate', new \DateTime((string) $startDate));
            }
            if ($endDate !== null && $endDate !== '') {
                $queryBuilder->andWhere('o.createdAt <= :endDate')
                    ->setParameter('endDate', new \DateTime((string) $endDate . ' 23:59:59'));
            }
    ...
    public function getPaginatedJobs(Request $request): JsonResponse
    {
        ...
        try {
            // Use JobRepository to query jobs
            ...
            // Apply sorting
            $allowedSortFields = ['createdAt', 'title', 'budget', 'status', 'id'];
            if (in_array($sortBy, $allowedSortFields, true)) {
                $sortOrder = strtoupper((string) $sortOrder) === 'ASC' ? 'ASC' : 'DESC';
