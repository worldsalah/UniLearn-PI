...
final class InstructorController extends AbstractController
{
    ...
    public function manageCourses(CourseRepository $courseRepository, UserRepository $userRepository): Response
    {
        ...
        // Temporarily show all courses to debug display issues
        $allCourses = $courseRepository->findAll();
        $userCourses = $courseRepository->findBy(['user' => $teacher]);

        // Debug: Log course information
        error_log('User ID: ' . $user->getId());
        error_log('User Email: ' . $user->getEmail());
        error_log('Found all courses: ' . count($allCourses));
        error_log('Found user courses: ' . count($userCourses));
        
        foreach ($allCourses as $course) {
            error_log('Course ID: ' . $course->getId() . ', Title: ' . $course->getTitle() . ', User ID: ' . ($course->getUser() ? $course->getUser()->getId() : 'null'));
        }

        // For now, show all courses to debug the display
        $courses = $allCourses;
    ...
    public function courseDetail(Course $course, UserRepository $userRepository): Response
    {
        ...
        // Allow viewing any course for now (ownership check can be added later)
        // if ($course->getUser() && $course->getUser()->getId() !== $user->getId()) {
    ...
    public function quiz(EntityManagerInterface $entityManager): Response
    {
        ...
        // Get all courses for quiz creation (not just user's courses)
        $allCourses = $entityManager->getRepository('App\Entity\Course')->findAll();
        
        // Also get user's courses for potential filtering
        $userCourses = $entityManager->getRepository('App\Entity\Course')->findBy(['user' => $user]);

        // Debug: Log course information
        error_log('Quiz Page - User ID: '.$user->getId());
        error_log('Quiz Page - All courses found: '.count($allCourses));
        error_log('Quiz Page - User courses found: '.count($userCourses));

        foreach ($allCourses as $course) {
            error_log('Quiz Page - Course: '.$course->getTitle().' (Status: '.$course->getStatus().', ID: '.$course->getId().', User ID: '.($course->getUser() ? $course->getUser()->getId() : 'null').')');
        }

        // For now, use all courses to allow quiz creation for any course
        $courses = $allCourses;

        // Get quizzes for this instructor's courses (from all courses for debugging)
        $quizzes = [];
        
        // DEBUG: Show all quizzes regardless of ownership to test display
        $allQuizzes = $entityManager->getRepository('App\Entity\Quiz')->findAll();
        error_log('DEBUG: Total quizzes in database: '.count($allQuizzes));
        
        foreach ($allQuizzes as $quiz) {
            error_log('DEBUG: Quiz - ID: '.$quiz->getId().', Title: '.$quiz->getTitle().', Course: '.($quiz->getCourse() ? $quiz->getCourse()->getTitle() : 'No Course'));
            $quizzes[] = $quiz;
        }
