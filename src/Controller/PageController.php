...
class PageController extends AbstractController
{
    ...
    {
        return $this->render('pages/help-center.html.twig');
    }
    
    #[Route('/contact-support', name: 'app_contact_support')]
    public function contactSupport(): Response
    {
        return $this->render('pages/contact-support.html.twig');
    }
    ...
    public function studentDashboard(EntityManagerInterface $entityManager, GamificationService $gamificationService): Response
    {
        ...
        $progress = $gamificationService->getProgressToNextLevel($user);
        ...
        // Get student's enrollments with progress data
        $enrollments = $enrollmentRepository->findBy(['user' => $user]);
        
        // Prepare enrolled courses with progress information
        $enrolledCourses = [];
        ...
        // Sort courses by progress (in progress first, then by enrollment date)
        usort($enrolledCourses, function($a, $b) {
            if ($a['progress'] === 100 && $b['progress'] < 100) return 1;
            if ($a['progress'] < 100 && $b['progress'] === 100) return -1;
            return $b['enrollment']->getEnrolledAt() <=> $a['enrollment']->getEnrolledAt();
        });
        ...
        $availableCourses = array_filter($allCourses, function($course) use ($enrolledCourseIds) {
            return !in_array($course->getId(), $enrolledCourseIds, true);
        });
        ...
        // Get user certificates (max 3)
        $certificates = $certificateRepository->findByUser($user);
        $recentCertificates = array_slice($certificates, 0, 3);
    ...
    #[Route('/student-quizzes', name: 'app_student_quizzes')]
    public function studentQuizzes(EntityManagerInterface $entityManager): Response
    {
        // Get the currently logged-in user
        $user = $this->getUser();
        ...
        // Get user's enrollments to filter quizzes by enrolled courses
        $enrollmentRepository = $entityManager->getRepository(\App\Entity\Enrollment::class);
        $enrollments = $enrollmentRepository->findBy(['user' => $user]);
        ...
        // Filter quizzes: show all quizzes or only those from enrolled courses
        $availableQuizzes = [];
        foreach ($quizzes as $quiz) {
            $quizCourse = $quiz->getCourse();
            // Include quiz if it has no course (general quiz) or if user is enrolled in the course
            if ($quizCourse === null || in_array($quizCourse->getId(), $enrolledCourseIds, true)) {
                $availableQuizzes[] = $quiz;
            }
        }

        return $this->render('student/quizzes.html.twig', [
            'user' => $user,
            'quizzes' => $availableQuizzes,
            'totalQuizzes' => count($availableQuizzes)
        ]);
    }
    ...
    public function studentProfile(EntityManagerInterface $entityManager, Request $request): Response
    {
        // Get currently logged-in user
        $user = $this->getUser();
        ...
        $quizResults = $quizResultRepository->findBy(['user' => $user], ['takenAt' => 'DESC'], 5);
        ...
        if (count($quizResults) > 0) {
            ...
            $avgScore = round($totalScore / count($quizResults), 1);
        }
    ...
    public function studentQuizAnalysis(EntityManagerInterface $entityManager, \App\Service\GeminiAiService $geminiAiService): Response
    {
        ...
        foreach ($quizResults as $result) {
            ...
            $quizData[] = [
                'quiz' => $quiz !== null ? $quiz->getTitle() : 'Unknown Quiz',
                'course' => $course !== null ? $course->getTitle() : 'Unknown Course',
                'score' => $result->getScore(),
                'max_score' => $result->getMaxScore(),
                'percentage' => $result->getMaxScore() !== null && $result->getMaxScore() > 0 && $result->getScore() !== null ? round(($result->getScore() / $result->getMaxScore()) * 100, 1) : 0,
    ...
    private function calculateBasicAnalytics(array $quizData): array
    {
        ...
        $improvementRate = 0;
        if (count($scores) > 1) {
            $midPoint = (int) floor(count($scores) / 2);
    ...
    public function getQuizAnalytics(EntityManagerInterface $entityManager): JsonResponse
    {
        ...
        foreach ($quizResults as $result) {
            ...
            $quizData[] = [
                ...
                'maxScore' => $result->getMaxScore(),
                'percentage' => $result->getMaxScore() !== null && $result->getMaxScore() > 0 && $result->getScore() !== null ? round(($result->getScore() / $result->getMaxScore()) * 100, 1) : 0,
                'takenAt' => $takenAt !== null ? $takenAt->format('Y-m-d H:i:s') : null,
                'createdAt' => $createdAt !== null ? $createdAt->format('Y-m-d H:i:s') : null
            ];
        }
    ...
    public function getCourseRecommendations(Request $request, EntityManagerInterface $entityManager): JsonResponse
    {
        ...
        try {
            ...
            $recommendedCourses = [];
            $goalLower = strtolower((string) $learningGoal);
    ...
    public function generateAiRoadmap(Request $request, EntityManagerInterface $entityManager, GeminiAiService $geminiAiService): JsonResponse
    {
        ...
        $userHistory = [
            'enrollments' => array_map(function($enrollment) {
                $course = $enrollment->getCourse();
                return [
                    'course_title' => $course !== null ? $course->getTitle() : 'Unknown Course',
                    'category' => $course !== null ? $course->getCategory()?->getName() : null,
                    'level' => $course !== null ? $course->getLevel() : null,
                    'progress' => $enrollment->getProgress(),
                    'status' => $enrollment->getStatus(),
            ...
            'quiz_results' => array_map(function($result) {
                $quiz = $result->getQuiz();
                return [
                    'quiz_title' => $quiz !== null ? $quiz->getTitle() : 'Unknown Quiz',
                    'course' => $quiz !== null ? $quiz->getCourse()?->getTitle() : null,
                    'score' => $result->getScore(),
                    'max_score' => $result->getMaxScore(),
                    'percentage' => $result->getMaxScore() !== null && $result->getMaxScore() > 0 && $result->getScore() !== null ? round(($result->getScore() / $result->getMaxScore()) * 100, 1) : 0,
        ...
        $userData = [
            ...
            'user_profile' => [
                ...
                'average_quiz_score' => count($quizResults) > 0 ? array_sum(array_map(fn($r) => $r->getMaxScore() !== null && $r->getMaxScore() > 0 && $r->getScore() !== null ? ($r->getScore() / $r->getMaxScore()) * 100 : 0, $quizResults)) / count($quizResults) : 0
            ]
        ];
    ...
    private function generateEnhancedRoadmap(array $userData, array $availableCourses, array $userHistory): array
    {
        ...
        usort($relevantCourses, function($a, $b) use ($level, $goal) {
    ...
    private function calculateCourseRelevance(array $course, int $week, string $level): float
    {
        ...
        if ($week <= 2 && in_array($course['level'], ['beginner', 'intro'], true)) {
            $baseScore += 0.1;
        } elseif ($week > 2 && in_array($course['level'], ['intermediate', 'advanced'], true)) {
            $baseScore += 0.1;
        }
    ...
    private function recommendActivities(int $week, string $focus, array $patterns): array
    {
        ...
        if (in_array('practical', $patterns['preferred_content_types'], true)) {
            $activities[] = 'Hands-on Projects';
        }
        
        if (in_array('visual', $patterns['preferred_content_types'], true)) {
    ...
    private function calculateEstimatedHours(int $week, string $level, array $activities): int
    {
        ...
        return (int) max(5, min(15, $baseHours + $activityHours));
    }
    ...
    private function courseMatchesGoal(string $title, string $description, string $category, string $goal): bool
    {
        ...
        foreach ($goalKeywords as $keyword) {
            if (in_array($keyword, $titleWords, true) || in_array($keyword, $descriptionWords, true)) {
                $matches++;
            }
        }
