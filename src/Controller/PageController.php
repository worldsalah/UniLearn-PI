...
class PageController extends AbstractController
{
    ...
    public function __construct(
        ...
        QuizStatisticsRepository $statsRepository,
    ...
    public function studentQuizzes(): Response
    {
        // Get the currently logged-in user
        $user = $this->getUser();
    ...
    #[Route('/student/quiz/{id}', name: 'app_student_take_quiz')]
    public function takeQuiz(int $id): Response
    {
        ...
        if (!$quiz->isOpen()) {
            ...
            if ($status === 'upcoming') {
                $this->addFlash('error', 'Ce quiz n\'est pas encore ouvert. Il sera disponible le ' . 
                    ($quiz->getOpeningDate() ? $quiz->getOpeningDate()->format('d/m/Y à H:i') : 'bientôt'));
            } elseif ($status === 'closed') {
                $this->addFlash('error', 'Ce quiz est maintenant fermé. La date de fermeture était le ' . 
                    ($quiz->getClosingDate() ? $quiz->getClosingDate()->format('d/m/Y à H:i') : 'récemment'));
            }
        ...
        return $this->render('student/take-quiz.html.twig', [
            'quiz' => $quiz,
            'questions' => $questions
        ]);
    }

    #[Route('/student/quiz/{id}/submit', name: 'app_student_submit_quiz', methods: ['POST'])]
    public function submitQuiz(int $id, Request $request): JsonResponse
    {
        ...
        foreach ($questions as $question) {
            ...
            if (isset($answers[$questionId]) && $answers[$questionId] === $question->getCorrectOption()) {
                $correctAnswers++;
            }
        }
        ...
        $message = $this->getMotivationalMessage($score);

        // Here you could save the quiz result to database if needed
    ...
    private function getMotivationalMessage(int $score): string
    {
        if ($score >= 90) {
            ...
            return "Très bien ! Vous avez de excellentes connaissances ! ⭐";
        } elseif ($score >= 70) {
            return "Bien joué ! Vous avez réussi le quiz ! 👏";
        } elseif ($score >= 60) {
            return "Pas mal ! Continuez à vous améliorer ! 💪";
        } elseif ($score >= 50) {
            return "Peut mieux faire. Révisez et réessayez ! 📚";
        } else {
            return "Besoin de plus de pratique. Ne abandonnez pas ! 🌱";
        }
    }
    ...
    public function adminQuizzes(Request $request): Response
    {
        ...
        $scoresChart = $this->createQuizScoresChart();
        $quizPerformanceChart = $this->createQuizPerformanceChart($quizzes);
        $questionsStatsChart = $this->createQuestionsStatsChart($allQuestions);

        // Debug: Log chart data
        error_log('Charts created: ' . count([$scoresChart, $quizPerformanceChart, $questionsStatsChart]));
        ...
        return $this->render('admin/quizzes.html.twig', [
            ...
            'quizPerformanceChart' => $quizPerformanceChart,
            'questionsStatsChart' => $questionsStatsChart,
        ]);
    }
...
<body>
    ...
    public function adminQuizDelete(int $id): JsonResponse
    {
        try {
            ...
            // Delete all quiz statistics for this quiz
            $quizStatistics = $this->quizStatisticsRepository->findBy(['quiz' => $quiz]);
    ...
    private function createQuizScoresChart(): Chart
    {
        $chart = $this->chartBuilder->createChart('quizScoresChart');

        // Récupérer les données de scores des quiz
        $stats = $this->statsRepository->createQueryBuilder('qs')
        ...
        error_log('Quiz scores stats: ' . json_encode($stats));
        ...
        $chart->setOptions([
            ...
            'plugins' => [
                ...
                'title' => [
                    'display' => true,
                    'text' => 'Distribution des scores des quiz',
                ],
            ],
            'scales' => [
                'y' => [
    ...
    private function createQuizPerformanceChart(array $quizzes): Chart
    {
        $chart = $this->chartBuilder->createChart('quizPerformanceChart');

        $labels = [];
        $data = [];
        
        foreach ($quizzes as $quiz) {
            $labels[] = $quiz->getTitle();
            
            // Calculer le score moyen pour ce quiz
            try {
                $avgScore = $this->statsRepository->createQueryBuilder('qs')
                    ->select('AVG(qs.score)')
                    ->where('qs.quiz = :quiz')
        ...
        error_log('Quiz performance data: ' . json_encode($data));
    ...
    private function createQuestionsStatsChart(array $allQuestions): Chart
    {
        $chart = $this->chartBuilder->createChart('questionsStatsChart');

        $totalQuestions = 0;
        $quizCounts = [];
        
        foreach ($allQuestions as $quizData) {
            $questionCount = count($quizData['questions']);
            $totalQuestions += $questionCount;
            $quizCounts[] = $questionCount;
        }

        // Distribution du nombre de questions par quiz
        $distribution = [
            '1-5 questions' => 0,
            '6-10 questions' => 0,
            '11-20 questions' => 0,
        ...
        foreach ($quizCounts as $count) {
            if ($count <= 5) {
                $distribution['1-5 questions']++;
            } elseif ($count <= 10) {
                $distribution['6-10 questions']++;
            } elseif ($count <= 20) {
                $distribution['11-20 questions']++;
            } else {
                $distribution['21+ questions']++;
            }
        }

        error_log('Questions distribution: ' . json_encode($distribution));
        ...
        $chart->setOptions([
            ...
            'plugins' => [
                ...
                'title' => [
                    'display' => true,
                    'text' => 'Distribution des questions par quiz',
                ],
            ],
            'scales' => [
                'y' => [
