...
class PriceIntelligenceService
{
    ...
    public function getMarketData(string $category): array
    {
        ...
        foreach ($prices as $price) {
            if ($price <= $p25) $distribution['budget']++;
            elseif ($price <= $p75) $distribution['standard']++;
    ...
    {
        if ($marketData['total_listings'] === 0) {
            return [
                'percentile' => 50,
                'rank' => 0,
                'total' => 0,
        ...
        elseif ($percentile <= 75) $position = 'standard';
    ...
    {
        if ($marketData['total_listings'] === 0) return 50;
        
        // Simple percentile calculation
        if ($price <= $marketData['min_price']) return 0;
        if ($price >= $marketData['max_price']) return 100;
