...
class VertexApiService
{
    ...
    /**
     * Build a prompt for Vertex AI based on parameters
     */
...
Return only a valid JSON array of these question objects. No additional text or explanation.";
    ...
    /**
     * Generate questions for specific course content
     *
     * @param string $courseTitle Course title
     * @param string $courseDescription Course description
     * @param int $questionCount Number of questions to generate
     * @param string $difficulty Difficulty level
     * @return array Response with generated questions
     */
    public function generateCourseQuestions(string $courseTitle, string $courseDescription, int $questionCount = 5, string $difficulty = 'medium'): array
    {
        try {
            $prompt = $this->buildCoursePrompt($courseTitle, $courseDescription, $questionCount, $difficulty);
            
            $response = $this->callVertexAI($prompt);
            
            $questions = $this->parseCourseQuestions($response, $courseTitle);
            ...
            $this->logger->error('Vertex AI Course Questions Error: ' . $e->getMessage(), [
                'exception' => $e,
            ...
            return [
                'success' => false,
                'error' => 'Failed to generate course questions using AI.',
                'details' => $e->getMessage()
            ];
        }
    }
    ...
    /**
     * Build prompt for course-specific question generation
     */
    private function buildCoursePrompt(string $courseTitle, string $courseDescription, int $questionCount, string $difficulty): string
    {
        return "You are an expert educational content creator. Generate {$questionCount} multiple-choice quiz questions for the following course:
...
REQUIREMENTS:
1. All questions must be directly related to the course content
2. Each question must have exactly 4 options (A, B, C, D)
3. Only one option should be correct
4. Questions should test important concepts from the course
...
Generate exactly {$questionCount} questions. Focus on practical knowledge and key concepts.";
    }

    /**
     * Parse course questions from AI response
     */
    private function parseCourseQuestions(array $response, string $courseTitle): array
    {
        $text = $response['candidates'][0]['content']['parts'][0]['text'];
